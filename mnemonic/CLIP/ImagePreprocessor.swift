...
nonisolated enum ImagePreprocessor {
    ...
    static func preprocess(_ image: NSImage) -> Data? {
        ...
        guard let croppedImage = resizedImage.cropping(
        ...
        pixelCtx.draw(croppedImage, in: CGRect(x: 0, y: 0, width: imageSize, height: imageSize))

        guard let pixelData = pixelCtx.data else { return nil }
        let pixels = pixelData.bindMemory(to: UInt8.self, capacity: imageSize * imageSize * 4)
